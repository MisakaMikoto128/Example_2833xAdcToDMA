...
// (other than DC component) is found at index 51 for i.e. around 1Khz the pass
// is set to 1 else fail is set to 1. So observe watch variables : "pass",
// "fail" and "max_idx". Also you can experiment by changing EPWM2 frequency and
// observing variation of max_idx correspondingly
//
//
// SIMULATED ADC INPUT:
...
extern uint16_t test_input[512];
extern const float test_golden[514];
...
void DSP_runTest(void) {
  ...
  for (i = (TEST_SIZE + (TEST_SIZE >> 1) + 1U), j = 0U; i <= ((TEST_SIZE << 1));
       i++, j++) {
    out.f32 = p_temp[j];
    gold.f32 = test_golden[i];
    err.f32 = fabsf(out.f32 - gold.f32);
